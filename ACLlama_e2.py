    ...
    def forward(
        ...
        if audio_tower is not None and (input_ids.shape[1] != 1 or self.training) and audios is not None:
            ...
            if True:
                ...
                #    num_patches = 500 #cur_audio_features.shape[0]
                #    audio_start_token_pos = torch.where(cur_input_ids == audio_config.audio_patch_token)[0][:1]
                #    #new_input_ids.append(torch.cat((cur_input_ids[:audio_start_token_pos + 500],cur_input_ids[audio_start_token_pos + num_patches:]),dim=0))
                #    cur_new_input_embeds = torch.cat((
                #        cur_input_embeds[:audio_start_token_pos],
                #        cur_audio_features[:num_patches],
                ...
                if self.training: 
                    for cur_input_ids, cur_input_embeds, shrink_feature in zip(input_ids, inputs_embeds, shrink_features):
                        pad_ids = torch.full(size=(num_patches-lengths.min(),), fill_value=audio_config.llm_pad_token_id, dtype=torch.long).to(attention_mask.device)
                        pad_embeds = self.embed_tokens(pad_ids)
                        v = shrink_feature.shape[0]

                        audio_start_token_pos = torch.where(cur_input_ids == audio_config.audio_patch_token)[0][:1]
                        cur_new_input_id = torch.cat((cur_input_ids[:audio_start_token_pos+v],cur_input_ids[audio_start_token_pos + num_patches:], pad_ids[:num_patches-v]),dim=0)
                        cur_new_input_embeds = torch.cat((
                        cur_input_embeds[:audio_start_token_pos],
                        shrink_feature,
                        cur_input_embeds[audio_start_token_pos + num_patches:],pad_embeds[:num_patches-v]), dim=0)
                        new_input_embeds.append(cur_new_input_embeds)
                        new_input_ids.append(cur_new_input_id)
                        label_shift.append(num_patches-v)
                    ...
                    for cur_input_ids, cur_input_embeds, shrink_feature in zip(input_ids, inputs_embeds, shrink_features):
                        ...
                        audio_start_token_pos = torch.where(cur_input_ids == audio_config.audio_patch_token)[0][:1]
                        cur_new_input_id = torch.cat((cur_input_ids[:audio_start_token_pos+v],cur_input_ids[audio_start_token_pos + num_patches:]),dim=0)
                        cur_new_input_embeds = torch.cat((
                        cur_input_embeds[:audio_start_token_pos],
                        shrink_feature,
                        cur_input_embeds[audio_start_token_pos + num_patches:]), dim=0)
                    ...
                    input_ids = torch.stack(new_input_ids, dim=0)
                    attention_mask=input_ids.ne(audio_config.llm_pad_token_id)
                    #print("after",attention_mask,input_ids)
                    inputs_embeds = torch.stack(new_input_embeds, dim=0)
        ...
        if labels is not None:
            ...
            if len(outputs["label_shift"]) >0:
                for i in range(len(outputs["label_shift"])):
                    ...
                    #print(shift_labels[i].shape,outputs["label_shift"][i])
